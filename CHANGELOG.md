...
### Added
- **Talking Points Agent**: New AI agent in Application Studio that transforms each tailored resume bullet into a compelling interview story using the Problem → Solved → Deployed framework
- Interview Story Guide artifact: full stories (90s to 3-4 min each) written in first person with natural, conversational tone -- not scripted
- Story Cheatsheet artifact: pocket-card quick-reference with one-line hooks, trigger keywords, and key metrics per story
- Talking Points runs automatically in both full and quick pipelines (after Tailor/Coach, before Strategist)
- Sources enrichment data from all resume variants (accomplishments, leadership indicators, scope metrics)
- Alembic migration 014: managed prompt seed for Talking Points agent
- **Story Bank**: Persistent personal library of interview stories that lives independently of job workspaces
- Individual story editing with version history (snapshot on each save)
- Fuzzy bullet matching: Talking Points agent automatically reuses existing stories for matching resume bullets, conserving AI token costs
- Story Bank UI at `/stories` with card grid, inline expand/edit, archive/restore, version history viewer
- Manual story creation for stories not generated by AI
- Bulk create endpoint for agent integration
