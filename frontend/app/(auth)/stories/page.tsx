...
export default function StoryBankPage() {
  ...
  // Selected story (replaces expandable cards)
  const [selectedStory, setSelectedStory] = useState<StoryBankStory | null>(null);
  ...
  useEffect(() => {
    if (selectedStory) {
      breadcrumbs.set([
        { label: "Story Bank", onClick: backToList },
        { label: selectedStory.story_title },
      ]);
    } else {
      breadcrumbs.set([{ label: "Story Bank" }]);
    }
  }, [selectedStory, breadcrumbs, backToList]);

  useEffect(() => {
    return () => breadcrumbs.clear();
  }, [breadcrumbs]);

  // Sidebar reset listener
  useEffect(() => {
    const handler = (e: Event) => {
      if ((e as CustomEvent).detail === "/stories") backToList();
    };
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <span className="font-medium">{row.getValue("story_title")}</span>
        ),
        enableColumnFilter: false,
      },
      {
        accessorKey: "source_company",
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title="Company" />
        ),
        cell: ({ row }) => row.getValue("source_company") || "—",
      ...
      {
        accessorKey: "proof_metric",
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title="Proof Metric" />
        ),
        cell: ({ row }) => {
          const metric = row.getValue("proof_metric") as string | null;
      ...
      },
      {
        ...
        cell: ({ row }) => row.getValue("times_used") as number,
        enableColumnFilter: false,
      },
      {
  ...
  const selectStory = useCallback(async (story: StoryBankStory) => {
    setSelectedStory(story);
    setEditingId(null);
    setShowVersions(false);
    setLoadingDetail(true);
    try {
      const data = await apiGet<StoryBankStoryDetail>(`/stories/${story.id}`);
      setDetail(data);
    } catch {
      // ignore
    } finally {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      setEditingId(null);
      if (selectedStory) await loadDetail(selectedStory.id);
      await loadStories();
    } finally {
      setSaving(false);
    }
  ...
  const handleArchive = async (id: string) => {
    ...
    await loadStories();
    await loadSummary();
    if (selectedStory?.id === id) backToList();
  };
    ...
    <div className="space-y-6">
      ...
      {selectedStory ? (
        <div className="space-y-4">
          <button
            onClick={backToList}
            className="inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm text-muted-foreground hover:bg-accent hover:text-accent-foreground"
          >
          ...
          <div className="rounded-lg border border-border bg-card p-6 space-y-4">
            ...
            ) : detail && editingId !== selectedStory.id ? (
              <>
                {/* Header */}
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="text-xl font-bold">{detail.story_title}</h2>
                  ...
                  {(detail.source_title || detail.source_company) && (
                    <p className="mt-1 text-sm text-muted-foreground">
                      {detail.source_title}{detail.source_title && detail.source_company ? " at " : ""}{detail.source_company}
                    </p>
                  )}
                  {detail.source_bullet && (
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    {detail.trigger_keywords?.map((kw) => (
                      <span
                        key={kw}
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-border">
                  {canEdit && (
                    <>
                      ...
                      <button
                        onClick={() => openAiModal(selectedStory)}
                      ...
                      {selectedStory.status === "active" ? (
                        ...
                        <button
                          onClick={() => handleRestore(selectedStory.id)}
